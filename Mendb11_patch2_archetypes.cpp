...
static void stylize_archetype_weights(int archetype_id, float* W1, float* b1, float* W2, float* b2)
{
  if(!W1 || !b1 || !W2 || !b2) return;

  float s1 = 1.0f, s2 = 1.0f, b1Shift = 0.0f, b2Shift = 0.0f;
  switch(archetype_id) {
    case ARCHE_ORDER:      s1 = 0.72f; s2 = 0.82f; b1Shift =  0.05f; b2Shift =  0.03f; break;
    case ARCHE_TURBULENCE: s1 = 1.45f; s2 = 1.35f; b1Shift = -0.02f; b2Shift =  0.00f; break;
    case ARCHE_GROWTH:     s1 = 1.12f; s2 = 1.24f; b1Shift =  0.03f; b2Shift =  0.08f; break;
    case ARCHE_DECAY:      s1 = 0.96f; s2 = 1.08f; b1Shift = -0.06f; b2Shift = -0.05f; break;
    case ARCHE_BALANCE:    s1 = 0.88f; s2 = 0.92f; b1Shift =  0.01f; b2Shift =  0.01f; break;
  }

  for(int i = 0; i < NN_H * NN_IN; i++) {
    float mod = 1.0f;
    if(archetype_id == ARCHE_TURBULENCE)
      mod = ((i % 3) == 0) ? 1.35f : (((i % 5) == 0) ? -1.10f : 1.0f);
    else if(archetype_id == ARCHE_GROWTH)
      mod = 1.0f + 0.25f * ((i % NN_IN) >= 2 ? 1.0f : -0.4f);
    else if(archetype_id == ARCHE_DECAY)
      mod = 1.0f - 0.28f * ((i % NN_IN) >= 4 ? 1.0f : 0.0f);
    W1[i] *= (s1 * mod);
  }

  for(int i = 0; i < NN_OUT * NN_H; i++) {
    float mod = 1.0f;
    if(archetype_id == ARCHE_ORDER)
      mod = ((i % NN_H) < (NN_H / 2)) ? 1.08f : 0.92f;
    else if(archetype_id == ARCHE_BALANCE)
      mod = ((i % 2) == 0) ? 1.05f : 0.95f;
    W2[i] *= (s2 * mod);
  }

  for(int i = 0; i < NN_H; i++)
    b1[i] = b1[i] * s1 + b1Shift;
  for(int i = 0; i < NN_OUT; i++)
    b2[i] = b2[i] * s2 + b2Shift;
}
...
static int init_all_archetypes()
{
  ...
  for(int k = 0; k < K_ARCHETYPES; k++) {
    ...
    stylize_archetype_weights(k, gHost_W1[k], gHost_b1[k], gHost_W2[k], gHost_b2[k]);
...
static void evolve_context(float phase, unsigned int seed)
{
  ...
  // Regime dynamics (Eq 22)
  float regime_raw = 0.9f * sinf(0.31f * phase) + 0.6f * cosf(0.17f * phase) + 0.35f * noise;
  gCtx.regime = tanhf(regime_raw);

  // Volatility dynamics (Eq 23) - simplified, no weight stats
  gCtx.volatility = clampf(0.5f + 0.15f * noise, -1.0f, 1.0f);

  // Trend dynamics (Eq 24)
  float trend_raw = 0.5f * sinf(0.12f * phase + 1.3f) + 0.20f * noise;
  gCtx.trend = tanhf(trend_raw);

  // Risk dynamics (Eq 25)
  float risk_raw = 0.8f * gCtx.regime - 0.6f * fabsf(gCtx.volatility) + 0.5f * gCtx.trend + 0.15f * noise;
...
static void compute_controller()
{
  ...
  logits[ARCHE_ORDER]     =  0.5f * gCtx.regime + 0.3f * gCtx.trend;
  logits[ARCHE_TURBULENCE]=  0.8f * gCtx.volatility - 0.2f * gCtx.regime;
  logits[ARCHE_GROWTH]    =  0.6f * gCtx.trend + 0.4f * gCtx.regime;
  logits[ARCHE_DECAY]     = -0.6f * gCtx.trend - 0.3f * gCtx.volatility;
  logits[ARCHE_BALANCE]   =  0.3f * gCtx.risk + 0.2f * (gCtx.regime + gCtx.trend);

  // Temperature modulation (Eq 56)
  gTemperature = 0.5f + 1.5f * sigmoid(3.0f * gCtx.volatility);
...
"  float vignette = clamp(1.15f - r, 0.0f, 1.0f);                                \n"
"  float stripe = 0.5f + 0.5f*sin(10.0f*(x + y) + phase + 2.0f*jitter);          \n"
"  float rcol = clamp(0.70f*blended[0] + 0.30f*stripe, 0.0f, 1.0f) * vignette;   \n"
"  float gcol = clamp(0.85f*blended[1] + 0.15f*(1.0f - stripe), 0.0f, 1.0f) * vignette;\n"
"  float bcol = clamp(0.75f*blended[2] + 0.25f*(0.5f + 0.5f*cos(8.0f*x - phase)), 0.0f, 1.0f);\n"
"                                                                                \n"
