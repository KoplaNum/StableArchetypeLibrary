...
static void compute_controller()
{
  ...
  float pulseSlow = 0.5f + 0.5f * sinf(0.23f * gPhase);
  float pulseFast = 0.5f + 0.5f * cosf(0.61f * gPhase + 1.2f * gCtx.regime);
  
  // Different archetype preferences based on context
  logits[ARCHE_ORDER]      =  0.55f * gCtx.regime + 0.25f * gCtx.trend + 0.35f * pulseSlow;
  logits[ARCHE_TURBULENCE] =  0.90f * gCtx.volatility - 0.15f * gCtx.regime + 0.30f * pulseFast;
  logits[ARCHE_GROWTH]     =  0.65f * gCtx.trend + 0.45f * gCtx.regime + 0.20f * sinf(0.39f * gPhase);
  logits[ARCHE_DECAY]      = -0.70f * gCtx.trend - 0.35f * gCtx.volatility + 0.20f * cosf(0.27f * gPhase);
  logits[ARCHE_BALANCE]    =  0.35f * gCtx.risk + 0.20f * (gCtx.regime + gCtx.trend) + 0.15f * (1.0f - pulseSlow);

  // Temperature modulation (Eq 56)
  gTemperature = 0.35f + 1.95f * sigmoid(2.6f * gCtx.volatility + 1.4f * (pulseFast - 0.5f));
